...
export const login = async (req, res) => {
  ...
  try {
    ...
    res.cookie("accessToken", accessToken, {httpOnly: true,secure: false, sameSite: "Strict", maxAge: 4 * 60 * 1000});
    res.cookie("refreshToken", refreshToken, {httpOnly: true, sameSite: "Strict", secure: false, maxAge: 120 * 60 * 1000});
...
export const refreshToken = async(req,res)=>{
  ...
  jwt.verify(refreshToken, process.env.REFRESH_SECRET_KEY, (err, user) => {
    ...
    res.cookie("accessToken", newaccessToken, {httpOnly: true,secure: false, sameSite: "Strict", maxAge: 4 * 60 * 1000});
